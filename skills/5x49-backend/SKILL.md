...
```
返回持久化 `EventRecord[]`，按时间倒序排列。`/library/events` 是实时 SSE；`/library/audit-events` 和 `/library/{movie_id}/audit-events` 是历史审计日志。`/library/audit-events` 可用 `aggregate_type`、`aggregate_id`、`type`、`command_id`、`correlation_id` 过滤，便于按一次操作追踪事件链。当前为混合模式：多数复杂流程仍旁路记录审计事件；`MovieIgnored`、`MovieMarkedMissing`、`MovieRestored`、`RootVideoOrganizationReverted`、`AnalysisStarted`、`AnalysisCompleted`、`AnalysisFailed` 等低风险状态变更会由事件同步投影到 `Movie` 当前状态表。扫描事件会去重：新记录写 `MovieDiscovered`，本地文件关键字段变化才写 `MovieFileObserved`，NFO 签名变化才写 `MovieMetadataParsedFromNfo`，missing 记录重新出现写 `MovieRestored`；成功文件夹扫描不再默认追加 `MovieFolderScanned`，但仍会通过 `/library/events` 发布实时刷新通知。事件类型包括 `MovieDiscovered`、`MovieFileObserved`、`MovieMetadataParsedFromNfo`、`MovieFolderScanned`、`MovieMarkedMissing`、`MovieIgnored`、`MetadataMatchSuggested`、`MetadataMatched`、`MetadataScrapeFailed`、`ArtworkDownloaded`、`ArtworkSelected`、`ArtworkRestored`、`NfoWritten`、`NfoRestored`、`RootVideoMoved`、`RootVideoMoveReversed`、`RootVideoOrganized`、`RootVideoOrganizationReverted`、`AnalysisStarted`、`AnalysisCompleted`、`AnalysisFailed`、`ExternalScoresRefreshed` 等。
...
`/library/operations/restore` 会先复用 dry-run 检查，再执行受支持的文件级补偿动作：`restore_poster`、`restore_nfo`、`reverse_root_move`。它会追加 `ArtworkRestored`、`NfoRestored` 或 `RootVideoMoveReversed` 补偿事件，不删除原事件，也不恢复当前 `Movie` 字段。若一次 root video organization 创建了新电影记录，反向移动成功后还会追加并投影 `RootVideoOrganizationReverted`，将该记录置为 `library_status=reverted`，避免后续扫描把它显示成普通 missing。
