...
`/library/operations/restore` 会先复用 dry-run 检查，再执行受支持的文件级补偿动作：`restore_poster`、`restore_nfo`、`reverse_root_move`。它会追加 `ArtworkRestored`、`NfoRestored` 或 `RootVideoMoveReversed` 补偿事件，不删除原事件，也不恢复当前 `Movie` 字段。
