...
import type { EventRecord, Job, LibraryFilmSummary } from "@/types/movie";
...
export default function EventSourcingHealthPanel() {
  ...
  const jobs = useSWR<Job[]>(API.jobs(), fetcher);
  const events = useSWR<EventRecord[]>(API.activityEvents({ limit: 100 }), fetcher);
  const loading = films.isLoading || jobs.isLoading || events.isLoading;
  const error = films.error || jobs.error || events.error;
  const invalidFilms = (films.data || []).filter((film) => !film.id || !film.primary_item?.id || film.primary_item.film_id !== film.id);
  const activeJobs = (jobs.data || []).filter((job) => ["queued", "running", "cancelling"].includes(job.status));
  ...
  const refresh = async () => Promise.all([films.mutate(), jobs.mutate(), events.mutate()]);
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 type-body text-ink-muted">Film/LibraryItem integrity, recoverable snapshots and background jobs.</p>
          </div>
        </div>
      ...
      {!loading && !error && (
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <Metric label="Failed jobs" value={failedJobs.length} warning={failedJobs.length > 0} />
